...
try {
  neonSql = require("@neondatabase/serverless").neon;
} catch (err) {
  neonSql = null;
}
...
function createFileStore() {
  ...
  return {
    ...
    warning: ephemeral
      ? "File store on Vercel uses /tmp and is not durable. Set POSTGRES_URL, KV_REST_API_URL, or BLOB_READ_WRITE_TOKEN."
...
function createPostgresStore() {
  const sql = neonSql(process.env.POSTGRES_URL);
  let ready = null;
  function readySql() {
...
function detectDriver() {
  ...
  if (process.env.POSTGRES_URL && neonSql) return "postgres";
  if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN && kvClient) return "kv";
  if (process.env.BLOB_READ_WRITE_TOKEN && blobPut) return "blob";
...
function createStore(driver) {
  switch (driver) {
    case "postgres":
      if (!process.env.POSTGRES_URL || !neonSql) {
        throw new Error("Postgres store requested but POSTGRES_URL / @neondatabase/serverless is missing");
      }
...
function describeStoreRequirements() {
  return {
    preferred: [
      "POSTGRES_URL — Vercel Postgres / Neon (preferred)",
      "KV_REST_API_URL + KV_REST_API_TOKEN — Vercel KV",
      "BLOB_READ_WRITE_TOKEN — JSON index on Vercel Blob",
    ],
    fallback: "Without those, the API uses a local/dev JSON file (data/bookings.json, or /tmp on Vercel — not durable).",
  };
}
