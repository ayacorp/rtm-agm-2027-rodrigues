...
const { buildGuestEmail, buildTreasurerEmail, buildContactEmail } = require("./email-templates");
...
function resendPayload(message) {
  const payload = {
    from: defaultFrom(),
    to: [message.to],
...
async function notifyContact(input) {
  ...
  try {
    const channel = await deliverAll([message]);
    if (channel) {
      return { sent: true, channel: channel, queued: false };
    }
  } catch (err) {
    console.error("[rtm-notify] contact send failed", err);
    return {
      sent: false,
      channel: "error",
      queued: false,
  ...
  return { sent: false, channel: "unavailable", queued: false };
}
