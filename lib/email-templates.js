...
const { publicBank, TBA_MESSAGE, treasurerPhone, notifyEmail } = require("./bank");
...
function buildContactEmail(input) {
  ...
  const body = String((input && input.message) || "").trim();
  const subject = "[RTM AGM 2027] Contact from " + name;
  const innerHtml = factsHtml([
    { k: "Name", v: name },
    { k: "Email", v: email },
  ])
    + '<p style="margin:18px 0 0;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.65;color:#123a44;white-space:pre-wrap;">'
    + escapeHtml(body)
    + "</p>";
  return {
    audience: "organiser",
    ...
    html: wrapHtml({
      title: subject,
      preheader: name + " sent a message from the AGM site.",
      eyebrow: "Contact us",
      heading: "A guest wrote in.",
      lede: "Reply to this email to write back to " + name + ".",
      innerHtml: innerHtml,
...
module.exports = {
  ...
  buildContactEmail: buildContactEmail,
};
