...
module.exports = wrap(async function proof(req, res) {
  ...
  const ref = String((parsed.fields.ref || "")).trim().toUpperCase();
  const email = String((parsed.fields.email || "")).trim().toLowerCase();
  ...
  const url = await uploadProof(ref, parsed.file);
  ...
  sendJson(res, 200, {
    ...
    bank: publicBank(),
    store: storeInfo(),
    notify: { sent: notify.sent, queued: notify.queued, channel: notify.channel },
  });
});
